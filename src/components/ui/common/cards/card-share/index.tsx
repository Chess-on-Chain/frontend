...
export default function CardShare({ type, title, icon, link }: CardShareProps) {
  ...
  return (
    <>
      <div
        className="w-full h-20 flex justify-between p-4 rounded-lg border border-neutral-800 bg-primary/60 cursor-pointer"
      ...
      {open && (
        <SharePopup type={type} link={link} onClose={() => setOpen(false)} />
      )}
    </>
  );
}
